...
def test_tile_bad_lod():
    with (
        ...
        patch("routers.data_tiles.load_slice", return_value=_make_ds()),
    ):
...
def test_tile_missing_date():
    with (
        patch("routers.data_tiles.get_lod_grids", return_value=_LOD_GRIDS),
        patch("routers.data_tiles.load_slice", side_effect=FileNotFoundError("No data")),
    ):
...
def test_manifest_ok():
    ...
    with (
        patch("routers.data_tiles.get_lod_grids", return_value=_LOD_GRIDS),
        patch("routers.data_tiles.load_slice", return_value=_make_ds()),
        patch("routers.data_tiles.render_manifest", return_value=payload),
    ):
...
def test_point_missing_date():
    with patch("routers.data_tiles.load_slice", side_effect=FileNotFoundError("No data")):
        response = client.get("/data_tiles/sea_level_anomaly/9999-01-01/point?lat=-35&lon=145")
    assert response.status_code == 404


def test_point_ok():
    with patch("routers.data_tiles.load_slice", return_value=_make_ds()):
...
def test_availability_ok():
    with (
        patch("routers.data_tiles.PRODUCTS", _FAKE_PRODUCTS),
        patch("routers.data_tiles.get_available_dates", return_value=["2024-06-01", "2024-07-01"]),
        patch("routers.data_tiles._three_months_ago", return_value="2024-01-01"),
    ):
...
def test_availability_date_filters():
    ...
    with (
        patch("routers.data_tiles.PRODUCTS", _FAKE_PRODUCTS),
        patch("routers.data_tiles.get_available_dates", return_value=all_dates),
    ):
...
def test_availability_no_dates_in_range():
    with (
        patch("routers.data_tiles.PRODUCTS", _FAKE_PRODUCTS),
        patch("routers.data_tiles.get_available_dates", return_value=["2020-01-01"]),
        patch("routers.data_tiles._three_months_ago", return_value="2024-01-01"),
    ):
        response = client.get("/data_tiles/manifest")
    assert response.status_code == 200
